...
| PR17 | Strategic-product SDK checks wave 1 — `databricks-sdk` pin 0.55.0 → 0.94.0; six new read-only checks (`serving_endpoint_exists`, `ai_gateway_configured`, `lakebase_instance_exists`, `lakebase_synced_table_online`, `vector_search_endpoint_exists`, `vector_search_index_ready`); `REQUIRED_PARAMS`/`KNOWN_PARAMS` contracts; linter validates check names + required params; check table in `docs/AUTHORING_QUEST_PACKS.md` | ✅ landed | #13 (`feature/gameday-pr17-sdk-checks-wave1`) |
| CP1–CP7 | The quest catalog — foundation packs fixed/extended (`lakehouse-foundations` 1.1.0, `ai-bi-gameday` 1.1.0; broken `ai-bi-intelligence-challenge` retired) + four new packs: `lakeflow-pipelines-gameday` (725 pts), `genie-deep-dive` (975), `ai-gateway-gameday` (900), `lakebase-roundtrip` (950), `agent-bricks-gameday` (900). All strict-lint clean with machine-playable `solutions`; SQL surfaces live-verified on a real warehouse via `scripts/verify_pack_sql.py`; catalog README with progression map + per-pack permissions matrix in `quest_packs/built_in/README.md` | ✅ landed | #14 #17 #19 #21 #24 #26 #27 |
| PR22 | Operator preflight harness + live integration tier — `pytest.ini` markers (default run excludes `integration`); `tests/integration/test_checks_live.py` env-gated live tier (**8/8 passed against a real workspace**: every SDK check answers correctly, scratch fixtures via the real warehouse); `scripts/preflight_event.py` plays a pack end-to-end against a deployed app (strict lint → import → event/team/join/start → bootstrap dry-run+execute → per task: execute `solutions` (SQL via warehouse, 8 workspace ops via SDK) → submit attempt → assert declared outcome → leaderboard sanity → reset/teardown rehearsal); documented as step 0 of every event | ✅ landed | #23 (`feature/gameday-pr22-preflight`) |
| PR21 | Workspace provisioning + whole-event teardown — `resources.workspace` pack schema (lakebase_instance / vector_search_endpoint / serving_endpoint / genie_space; per-event or per-team via `${team_slug}`); namespace prefix guard `quest-<slug>-` (`assert_resource_name_in_namespace`) as the sole authority; plan builder with cost caps (1 Lakebase, 1 VS, 32 endpoints); `SDKWorkspaceExecutor` (injectable seam); deletion doubly gated (registry-recorded AND namespace-prefixed); bootstrap/plan endpoints gain the workspace lane; new `POST …/resources/teardown` (workspace resources + team schemas + the event catalog, confirm-required, dry-runnable); `scripts/provision_event.py` operator CLI | ✅ landed | #20 (`feature/gameday-pr21-workspace-provisioning`) |
| PR20 | Strict playability lint + `solutions` schema — `--strict` CLI flag / `strict: true` lint-API option; rules: auto-validated tasks must carry host-only machine-executable `solutions` steps (`sql`/`workspace_op`/`skip`), SDK/REST validators must pair with a manual fallback, manual-only tasks must set `manual_validation_required`, unknown check names error; CI strict-lints every shipped pack; all 3 catalog packs retrofitted with `solutions` and strict-clean. (`resources.workspace` schema deferred to PR21 so schema + execution land together.) | ✅ landed | #18 (`feature/gameday-pr20-strict-lint`) |
| PR19 | `rest_api` validator + vocabulary tightening — query a serving endpoint by **name** (URL/headers/auth are lint+runtime errors) with a host-authored prompt and evaluate the reply via the shared expectation engine; handles chat (`messages`) and agent/Responses (`input`) payloads with auto-fallback (**verified live against a real KA endpoint: `passed`**); clamps max_tokens ≤ 512, prompt ≤ 4k chars, timeout ≤ 60s. `KNOWN_VALIDATOR_TYPES` tightened: `system_table`/`notebook`/`python_code` removed (no backend = the stub failure mode); unknown types are now lint **errors**. Validator-library doc rewritten to executable-only patterns | ✅ landed | #16 (`feature/gameday-pr19-rest-api-validator`) |
| PR18 | SDK checks wave 2 + live API spikes — `genie_space_curated` + `genie_conversation_started` (serialized-space export + conversation listing, payload shape **verified against a live workspace**), `lakebase_app_connected`, and first-class Agent Bricks checks `knowledge_assistant_exists` / `multi_agent_supervisor_exists` via the beta `/api/2.0/tiles` REST surface (**spike succeeded live**: tile list/detail with `tile_type` KA/MAS + `serving_endpoint_name` — the Agent Bricks pack gate is satisfied) | ✅ landed | #15 (`feature/gameday-pr18-checks-wave2`) |
...
- **Review follow-ups (cleanup-grade, from the pre-merge stack review)** — unify the 7 `${...}` template-resolver implementations into one shared module (3 incompatible slot grammars exist today); factor the list-filter-qualify skeleton shared by 5 SDK checks; memoize per-attempt workspace listings (genie/serving) on the hot submission path and consider offloading `rest_api` model calls to a thread; derive the preflight's expected-status from `aggregate_status` instead of a parallel rule; make `WORKSPACE_CAPS` env-overridable; select `rest_api` payload format from the endpoint's declared task type instead of error-message sniffing; per-check validator outcome counters on the host overview.
